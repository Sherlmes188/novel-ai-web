  ...
  return (
    <AppShell title="小说项目">
      <div className="mb-5 flex items-center justify-between gap-3">
        <p className="muted">管理你的长篇项目、章节草稿和 AI 任务。</p>
        ...
        <form action={batchGenerateLatestChaptersAction} className="panel mb-5 grid gap-4 p-5">
          ...
          <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
            {novels.map((novel) => {
              ...
              return (
                <label key={novel.id} className="rounded-lg border border-[#ded8ca] bg-[#fffdf8] p-3 text-sm">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {novels.map((novel) => (
          <Link key={novel.id} href={`/novels/${novel.id}`} className="panel block p-5 hover:border-[#28726a]">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-black">{novel.title}</h2>
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-sm">
              <span><b>{novel.chapters.length}</b><br /><span className="muted">章节</span></span>
              <span><b>{novel.currentWords}</b><br /><span className="muted">字数</span></span>
              <span><b>{formatDate(novel.updatedAt)}</b><br /><span className="muted">更新</span></span>
            </div>
          </Link>
        ))}
      </div>
